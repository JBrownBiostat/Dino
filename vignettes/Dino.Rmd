...
## Functions ##
###############
gamDens.func <- function(y, lamVec, tauVec, phi, n = 1e3) {
  x <- seq(
    quantile(y, 1 / (2 * n)),
    quantile(y, 1 - 1 / (2 * n)),
    length = n
...
plotSim.func <- function(muVec, dVec, k) {
  n <- k ^ 2
  LS <- c()
  sampY <- c()
  ...
  dinoOut <- dinoFit.func(sampY, LS)
  
  dinoDens <- gamDens.func(sampY, dinoOut$lamPar, dinoOut$tauPar, dinoOut$phi)
  ...
  p2 <- ggplot(plotDat, aes(x = x, y = y, color = model)) + 
    ...
    labs(x = "Expression", y = "Density", color = "Model", title = "Reference vs.\nFitted distribution") + 
    themeObj
  xInd <- which.max(y.dens < 1e-3 * max(y.dens) & x.dens > x.dens[which.max(y.dens)])
  if(xInd > 1) {
    p2 <- p2 + 
      xlim(c(0, x.dens[which.max(y.dens < 1e-3 * max(y.dens) & x.dens > x.dens[which.max(y.dens)])]))
  }
  ...
  p1 <- ggplot(plotDat, aes(x = LS, y = y)) + 
    ...
    labs(x = "log-Depth", y = "Expression (log)", title = "Fitted means", fill = "Count\n(log10)")
  for(i in 1:length(dinoOut$lamPar)) {
    p1 <- p1 + 
      geom_abline(slope = 1, intercept = log1p(dinoOut$lamPar[i]), col = 1, alpha = 0.5, lwd = 0.75)
  }
  for(i in 1:length(muVec)) {
    p1 <- p1 + 
...
```{r Multimodal Simulation, echo = F}
p100 <- plotSim.func(c(5, 20), c(0.2, 0.1), 100)
```
